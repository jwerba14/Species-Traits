...
parameters {
  real<lower=0> tau;
  real<lower=0> log_alpha_bar;
  real<lower=0> log_sigma_alpha;
  real<lower=0> log_beta_bar;
...
transformed parameters {
  ...
  real beta[L+1];
  for(i in 1:L+1){
      alpha[i] = exp(log_alpha_bar) + exp(log_sigma_alpha)*eps_alpha[i];
      beta[i] = exp(log_beta_bar) + exp(log_sigma_beta)*eps_beta[i];
  }
...
model {
  ...
  log_alpha_bar ~ lognormal(0, 0.5);
  log_sigma_alpha ~ lognormal(0,1);
  log_beta_bar ~ lognormal(0,0.5); 
