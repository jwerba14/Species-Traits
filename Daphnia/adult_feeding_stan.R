...
## this is wrong- do calculation after calc rate per min then multiply by 3600 min/day
cont <- rdat %>% 
  mutate(Chl_Time_Diff_day = (Chl_Time_Diff/60)*24,Nh4_Time_Diff_day = (Nh4_Time_Diff/60)*24) %>%
  filter(control == 2) %>% 
  mutate(chl_diff =(chl1-chl2)/Chl_Time_Diff_day, nh4_diff= (nh41-nh42)/Nh4_Time_Diff_day) %>% # subtract 1 from 2 for change over time
  group_by(Treatment) %>%
