...
fit_nls <- function(subdat, par_names=c("r","k")) {
  ...
  nls_fit <- try(
    nlxb(Population ~  k/ (1+((k/3.35)-1)*exp(-r*Day)),data=subdat,start = list(r=1,k=50)),
    silent = TRUE
  )
...
all <- left_join(final, cerio, by= "Rep")
ggplot(data = all, aes(Treatment, r_est)) + geom_point()
ggplot(data = all[all$Treatment != 75,], aes(Treatment, k_est)) + geom_point()
