...
#need saturation function - will need to have different parameters for each saturation funciton in ODE
sat_fun <- function(a, s, p) {
...
> full_equations <-function(t, state, parameters) {
  ...
  # rate of change
      ...
      dammonium <- daph_a*sat_fun(a,b,algae)*r + daph_j*sat_fun(c,d,algae)*g+ 
        cerio*sat_fun(e,f,algae)*y +
        # also add a constant released based on density dependent death
        daph_j*deathj*cj + daph_a*deatha*cda + cerio*deathc*cc - algae*deatha*ca-
        # lose from uptake and a density dependent (?) from air
        algae*sat_fun(t,h,ammonium) - cammonium*ammonium
      ...
      dnitrate <- n*ammonium -
        ...
        algae*sat_fun(p,q, nitrate) - cnitrate*nitrate
      ...
      ddaph_j <- sat_fun(z,w,algae)*daph_a -
        ...
        daph_j*deathj - sat_fun(l,m,algae)*daph_j
      
        #daphnia adults are added from growth by juveniles and are lost from death
      ddaph_a <- sat_fun(l,m,algae)*daph_j- daph_a*deatha
        
        #ceriodaphnia are added by birth and are lost from death
      dcerio <- sat_fun(g,h,algae)*cerio - cerio*deathc
     
        # algae is added from growth based on uptake of ammonium and nitrate
       dalgae <- algae*sat_fun(a,s,ammonium) + algae*sat_fun(p,q, nitrate) -
        # algae is lost from being eaten by each of the 3 zooplankon
         -daph_a*sat_fun(a,b,algae) + daph_j*sat_fun(c,d,algae)+cerio*sat_fun(e,f,algae) -
