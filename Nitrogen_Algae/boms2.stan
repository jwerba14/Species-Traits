functions { 
  real[] gfun(real t, 
    ...
    dydt[1] = -y[2] * y[1] * params[1] * params[3]/(params[3] + y[1]) + params[4] *     (params[5] * y[2] + params[6] * (y[2]^2)) - 0.04085 * y[1]; 
    dydt[2] = params[2] * y[2] * y[1] * params[1] * params[3]/(params[3] +     y[1]) - params[5] * y[2] + params[6] * (y[2]^2); 

    return dydt;
  }

  real[] ifun(real t, real t0, real alpha, real beta, real omega, real gamma, real death1, real death2, real delta, real[] y0) { 
    real y_ini[2]; 
    real params[7];
    params[1] = alpha; 
    params[2] = beta; 
  ...
  real[] simfun(real t, real t0, real alpha, real beta, real omega, real gamma, real death1, real death2, real delta, real[] y0, real[] x_r, int[] x_i){
    real y_hat[1,2];
    real params[7];
    params[1] = alpha; 
    params[2] = beta; 
  ...
  real mufun(real t, real t0, real alpha, real beta, real omega, real gamma, real death1, real death2, real delta, real[] y0, real[] y_hat){
    return ((1 - delta) * y_hat[2] + delta * y_hat[1]);
  }

  real tmpfun(real t, real t0, real alpha, real beta, real omega, real gamma, real death1, real death2, real delta, real[] y0, real mu_hat) { 
    return mu_hat;
  }
} 
data { 
  ...
  int<lower=1> K_death2;  // number of population-level effects
  matrix[N, K_death2] X_death2;  // population-level design matrix
  // covariate vectors
  vector[N] C_1;
  vector[N] C_2;
  vector[N] C_3;
...
parameters { 
  ...
  vector[K_death2] b_death2;  // population-level effects
...
transformed parameters { 
  ...
  vector[N] nlp_death2 = X_death2 * b_death2;
  vector[N] mu;
  ...
  for (n in 1:N) { 
    ...
    nlp_death2[n] = exp(nlp_death2[n]); 
    if (which_t0[n]==n) {
    // compute non-linear predictor
      y0[n,] = ifun(C_1[n] , C_2[n] , nlp_alpha[n] , nlp_beta[n] , nlp_omega[n] , nlp_gamma[n] , nlp_death1[n] , nlp_death2[n] , C_3[n], rep_array(0.1, 0));
    } else {
      y0[n,] = y_hat[which_t0[n],];
    }
    // compute non-linear predictor
    y_hat[n,] = simfun(C_1[n] , C_2[n] , nlp_alpha[n] , nlp_beta[n] , nlp_omega[n] , nlp_gamma[n] , nlp_death1[n] , nlp_death2[n] , C_3[n], y0[n,], x_r, x_i);
    // compute non-linear predictor
    mu[n] = mufun(C_1[n] , C_2[n] , nlp_alpha[n] , nlp_beta[n] , nlp_omega[n] , nlp_gamma[n] , nlp_death1[n] , nlp_death2[n] , C_3[n], y0[n,], y_hat[n,]);
  } 
} 
model { 
  ...
  target += exponential_lpdf(exp(b_gamma) | 0.1)
    - 1 * exponential_lccdf(0 | 0.1);
  target += exponential_lpdf(exp(b_death1) | 0.1)
    - 1 * exponential_lccdf(0 | 0.1);
  target += exponential_lpdf(exp(b_death2) | 0.1)
    - 1 * exponential_lccdf(0 | 0.1);
  target += (b_alpha);
  target += (b_beta);
  target += (b_omega);
